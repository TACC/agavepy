...
import os
import getpass
from ..constants import (PLATFORM, ENV_USERNAME, ENV_PASSWORD,
                         ENV_TOKEN, ENV_REFRESH_TOKEN)
from .tty import print_stderr

__all__ = ['prompt_password', 'prompt_username',
           'prompt_access_token', 'prompt_refresh_token']
...
def prompt_refresh_token(refresh_token=None, quiet=False):
    if refresh_token is not None and refresh_token != '':
        if not quiet:
            print_stderr(WARNING_MESSAGE.format(
                'a refresh token', ENV_REFRESH_TOKEN))
