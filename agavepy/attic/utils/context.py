...
ALLOWED_KEYS = ('tenantid', 'baseurl', 'username', 'apikey', 'apisecret',
                'client_name', 'expires_at', 'expires_in', 'created_at',
                'access_token', 'refresh_token', 'devurl')

DEFAULT_KEYS = ('tenantid', 'baseurl', 'username', 'access_token',
                'refresh_token', 'apikey', 'apisecret')
...
def bootstrap_context(cache_dir=None, precedence='sessions', **kwargs):
    ...
    try:
        client_context = _context_from_client_file(client_file, **kwargs)
    except IOError:
