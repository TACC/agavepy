...
API_BLACKLIST = ('apps.getJobSubmissionForm',
                 ...
                 'systems.deleteCredentialsForInternalUser',
                 'systems.listCredentialsForInternalUser',
                 'systems.updateCredentialsForInternalUser',
                 'systems.deleteCredentialsForInternalUserByType',
                 'systems.listCredentialsForInternalUserByType',
                 'systems.updateCredentialsForInternalUserByType')

# Jina is told to treat all of HERE as a templates
# directory so this is essentially a placeholder
RESOURCES = ('resources.json.j2', 'abaco_resources.json.j2',
...
def schemify(name, classmod, dest_path):
    ...
    try:
        ...
        jsf = open(os.path.join(dest_path, schema_file_name), 'w')
        jsf.write(jsp + '\n')
        jsf.close()
...
def load_resource(resource_file, api_server='https://api.tacc.cloud'):
    ...
    :rtype: dict
    """
    conf = ConfigGen(os.path.basename(resource_file))
    env = jinja2.Environment(loader=jinja2.FileSystemLoader(
        os.path.dirname(resource_file)),
                             trim_blocks=True,
                             lstrip_blocks=True)
    rsrcs = json.loads(
        conf.compile(
            {'api_server_base': urllib.parse.urlparse(api_server).netloc},
            env))
    return rsrcs
...
def main():
    ...
    os.makedirs('docs/_static/schema', exist_ok=True)
    specs = os.listdir('agavepy/resources')
    specs = [s for s in specs if s.startswith('api')]
    for rt in specs:
        rspath = os.path.join(PWD, 'agavepy', 'resources', rt)
        resc = load_resource(rspath)

        # Write the JSON document
        rsfile = rt.replace('.j2', '')
        with open(os.path.join('docs/_static/openapi/1.2', rsfile), 'w') as sw:
            sw.write(json.dumps(resc, indent=4, sort_keys=True))
            sw.close()

        api = resc.get('apis')[0]
        api_name = api.get('name')
        if api_name not in SVC_BLACKLIST:
            os.makedirs('docs/docsite/{0}'.format(api_name), exist_ok=True)
            rst_file = 'docs/docsite/{0}/{0}.rst'.format(api_name)
                        ...
                        if submod_name_path not in API_BLACKLIST:
                            ...
                            if resp_type in respmods:
                                ...
                                if resp_type_obj == 'array':
                                    # get item $ref
                                    resp_item = respmods[resp_type]['result'][
                                        'items']['$ref']
    ...
    # res = load_resource('resources/deprecated_resources.json.j2')
