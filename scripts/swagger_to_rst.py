...
def load_resource(resource_file, api_server='https://api.tacc.cloud'):
    ...
    env = jinja2.Environment(loader=jinja2.FileSystemLoader(os.path.dirname(resource_file)),
                ...
                #for op in sub_api.get('operations'):
                    ...
                    if submod_name_path not in API_BLACKLIST:
                        ...
                        for param in op.get('parameters'):
                            ...
                            if param['required']:
                                arglist.append(param['name'] + '=<{0}>'.format(param['name'].upper()))
                            else:
                                # use to extract exemplar JSON where provided
                                default = param.get('defaultValue', None)
                                kwarglist.append(param['name'] + '={0}'.format(default))
                        ...
                        f.write(rst_heading('Keyword Args:',
                                            'subsection') + '\n')
                        for fp in fmtlist:
                            # compute type
                            fmttype = fp['jtype']
                            ...
                            if fp['jtype'] != 'string':
                                if fp['jtype'] in classmods:
                                    ...
                                    js = schemify(fp['jtype'],
                                                classmods[fp['jtype']], 'docs/_static/schema')
                                    f.write(js)
